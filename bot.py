...
IMAGE_PANEL  = "https://cdn.discordapp.com/attachments/1497777455199682580/1505353425896865932/grzue2pxqaa94te.jpg?ex=6a0a5120&is=6a08ffa0&hm=f6b288bb8f7f720cb2e28a3ceeb9af8acec7663a9dbfa8aefd83f0cb6fc28c0c&"
IMAGE_TICKET = "https://cdn.discordapp.com/attachments/1497777455199682580/1505353425896865932/grzue2pxqaa94te.jpg?ex=6a0a5120&is=6a08ffa0&hm=f6b288bb8f7f720cb2e28a3ceeb9af8acec7663a9dbfa8aefd83f0cb6fc28c0c&"
...
# ── Вспомогательная функция: поиск тикета игрока ──────────────────────
def get_existing_ticket(guild, user):
    """Вернуть уже открытый тикет пользователя или None."""
    expected_name = f"тикет-от-{user.name}".lower().replace(" ", "-")
    return discord.utils.get(guild.text_channels, name=expected_name)
...
class TicketCategorySelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # ── ФИКС: проверка дубля тикета до создания ───────────────────
        existing = get_existing_ticket(guild, user)
        if existing:
        ...
        # Имя канала совпадает с тем, что проверяет get_existing_ticket
        ...
        if log_ch:
            log_embed = discord.Embed(title="📂 Новый тикет", color=discord.Color.green())
...
class OpenTicketView(View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: Button):
        existing = get_existing_ticket(interaction.guild, interaction.user)
        if existing:
...
async def ticket_panel(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Время работы поддержки с 12:00 - 22:00 МСК"
        ),
        color=discord.Color.blurple()
    )
...
async def voice_setup(ctx):
    ...
    await ctx.send("✅ Войс-лобби создано!", delete_after=5)
